...
const ToDoListItem = ({ name, id, completed, clearInput }) => {
    ...
    return (
        <li className='to-do flex items-center special-background max-md:py-[15px] max-md:px-[18px] bg-light-veryLightGray dark:bg-dark-veryDarkDesaturatedBlue text-dark-darkGrayishBlue border border-light-veryLightGrayishBlue rounded-none dark:border-dark-veryDarkGrayishBlue2 cursor-pointer'>
            <button type='button' className='shrink-0 check-btn flex justify-center items-center w-6 h-6 rounded-full border border-light-veryLightGrayishBlue dark:border-dark-veryDarkGrayishBlue2 duration-500 ease-out' onClick={onComplete}>
                {completed ? (
            ...
            <div className='w-[400px] max-md:w-[380px] max-sm:w-[300px] max-xs:w-[200px]'>
                <p className='ms-5 max-md:text-[15px] text-base text-light-veryDarkGrayishBlue dark:text-dark-lightGrayishBlue break-all'>{name}</p>
            </div>
            <div className='ml-auto flex'>
                <button className='w-5 ml-2 mr-5 edit-btn shrink-0' onClick={onEdit}>
                ...
                <button className='delete-btn shrink-0' onClick={onDelete}>
